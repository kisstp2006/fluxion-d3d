...
//!
//! Sibling of `d3d12.zig`, split out so that file stays the device and the
//! queue: everything here is what a renderer allocates once it has both.
//!
...
/// `fluxion-rhi`'s Direct3D 11 backend for the rest of the table; the two
/// libraries do not share a type, and pinning the few this one calls is
/// simpler than factoring a `dxgiformat.zig` out for one caller.
...
/// `D3D12_CPU_DESCRIPTOR_HANDLE`. One `SIZE_T`, exactly as the header lays it
/// out - and see the module comment for why the methods that hand one back
/// are not simply "return this".
...
/// `D3D12_HEAP_FLAGS`. None of the bits are used by this backend - a
/// committed resource with the default flags is a heap and a resource made
/// together, which is all the MVP needs - so this stays a bare `u32` rather
/// than a `packed struct` with thirty reserved bits.
...
/// `D3D12_RESOURCE_FLAGS`. Every bit this backend never sets is still named,
/// so the type says what a resource is not asking for as plainly as what it
/// is.
...
pub const ResourceDesc = extern struct {
    ...
    /// pinned to when `Dimension` is `BUFFER` - see the struct's doc comment
    /// in the SDK headers, which this mirrors.
...
/// `D3D12_RESOURCE_STATES`. Not exhaustive - not every state this backend
/// never asks for is named - but every one it does ask for has the header's
/// number, because two states silently swapped is a validation layer warning
/// at best and a hazard the driver does not catch at worst.
...
/// never creates a depth-stencil committed resource, so the
/// `D3D12_DEPTH_STENCIL_VALUE` half is not declared.
pub const ClearValue = extern struct {
    format: Format,
    color: [4]f32 = .{ 0, 0, 0, 0 },
};

/// `D3D12_RANGE`. `Begin == End` means "nothing is read", which is the
/// pattern an upload buffer that is only ever written maps with.
...
/// every ordinary shader resource view wants, computed by hand once here
/// rather than re-derived at every call site. `0x1688 == 5768`: bits 0-2 are
/// component 0 read from source channel 0, bits 3-5 component 1 from channel
/// 1, and so on, with bit 12 always set (`ALWAYS_SET_BIT_AVOIDING_ZEROMEM_MISTAKES`,
/// the SDK's own name for it) so an all-zero mapping is never mistaken for a
/// valid one.
...
/// `D3D12_TEX2D_RTV`: `{ MipSlice, PlaneSlice }`, the only render target view
/// this backend ever makes - one of a swap chain's back buffers.
...
/// `fluxion-rhi`'s Direct3D 11 backend's `FilterMode` for how the bits are
/// built from a caller's min/mag/mip choice; this backend reuses that shape.
...
/// value (see the module comment on `d3d12.zig`), and this backend never
/// calls the rest. `GetGPUVirtualAddress` is safe to declare plainly: its
/// return is `D3D12_GPU_VIRTUAL_ADDRESS`, a `typedef UINT64`, not a class
/// type, so MSVC returns it in a register like any other scalar.
...
/// `ID3D12Resource`'s; `heapStart` below is the one safe way to reach the two
/// slots after it.
...
/// event it would need are not declared, matching the plan's "poll-spin is
/// fine" choice.
...
pub fn createDescriptorHeap(device: *ID3D12Device, desc: DescriptorHeapDesc) Error!*ID3D12DescriptorHeap {
    ...
    const result = device.vtable.CreateDescriptorHeap(device, &desc, com.iidOf(ID3D12DescriptorHeap), &raw);
...
pub fn createCommittedResource(
    ...
    var raw: ?*anyopaque = null;
    const result = device.vtable.CreateCommittedResource(
        device,
        &heap_properties,
        heap_flags,
        &desc,
...
pub fn createFence(device: *ID3D12Device, initial_value: u64, flags: FenceFlags) Error!*ID3D12Fence {
    var raw: ?*anyopaque = null;
    const result = device.vtable.CreateFence(device, initial_value, flags, com.iidOf(ID3D12Fence), &raw);
...
pub fn createConstantBufferView(device: *ID3D12Device, desc: ?*const ConstantBufferViewDesc, dest: CpuDescriptorHandle) void {
    device.vtable.CreateConstantBufferView(device, desc, dest);
}
...
pub fn createShaderResourceView(device: *ID3D12Device, resource: ?*ID3D12Resource, desc: ?*const ShaderResourceViewDesc, dest: CpuDescriptorHandle) void {
    device.vtable.CreateShaderResourceView(device, resource, desc, dest);
}
...
pub fn createRenderTargetView(device: *ID3D12Device, resource: ?*ID3D12Resource, desc: ?*const RenderTargetViewDesc, dest: CpuDescriptorHandle) void {
    device.vtable.CreateRenderTargetView(device, resource, desc, dest);
}
...
/// file does not also need that one.
pub const descriptorHandleIncrementSize = d3d12.descriptorHandleIncrementSize;

/// The staging layout, row count and pitch `CopyTextureRegion` needs to
/// upload into `resource`'s subresources from a buffer: `d3d12.zig`'s
/// `ID3D12Device.VTable.GetCopyableFootprints`, given a real signature here
/// because every parameter but the resource description is an output this
/// module already has types for.
///
/// Every output pointer may be null - "I don't want this one" - except that
/// asking for none of them is pointless. `total_bytes` is the size the
/// staging buffer must be at least.
pub fn getCopyableFootprints(
    ...
    device.vtable.GetCopyableFootprints(device, desc, first_subresource, num_subresources, base_offset, layouts, num_rows, row_size_bytes, total_bytes);
}
...
test "the descriptions the runtime reads are shaped as it expects" {
    try testing.expectEqual(@as(usize, 8), @sizeOf(usize)); // this file assumes a 64-bit host, as the rest of fluxion-d3d does
...
fn warpDeviceOrSkip(lib: D3d12, dxgi_lib: *dxgi.Dxgi) !*ID3D12Device {
...
test "a descriptor heap, and the CPU handle at its start" {
    ...
    const stride = d3d12.descriptorHandleIncrementSize(device, .rtv);
