...
//!
//! `d3d12.zig`'s own `RootSignatureDesc` already has the right shape for the
//! empty signature it tests with; `serializeGraphicsRootSignature` below
//! builds one with real parameters in it without changing that type - the
//! `parameters` field is `?*const anyopaque` there and stays that way, cast
//! into on the way in, exactly as a `void*` in the C header is.
...
/// `D3D12_ROOT_CONSTANTS`. Declared for layout completeness - it is one of
/// the three shapes `RootParameterUnion` must be exactly as wide as - and
/// never populated: this backend passes uniforms through constant buffers,
/// not inline root constants.
...
/// whose widest member is `DescriptorTable` (it holds a pointer and needs
/// 8-byte alignment, so the union starts at offset 8 and is itself 16 bytes),
/// then `ShaderVisibility` - modelled as an `extern union` of the three real
/// variants rather than approximated by hand, the same pattern
...
/// Serialise a root signature with real parameters. `d3d12.zig`'s
/// `RootSignatureDesc.parameters` is `?*const anyopaque`, exactly the shape a
/// `const void*` field in the header has, so a typed array goes in with one
/// cast and the struct that already has a passing test is untouched.
pub fn serializeGraphicsRootSignature(lib: d3d12.D3d12, params: []const RootParameter, flags: u32) Error!*d3d12.ID3DBlob {
...
pub fn createRootSignature(device: *ID3D12Device, node_mask: u32, blob: []const u8) Error!*ID3D12RootSignature {
    ...
    const result = device.vtable.CreateRootSignature(device, node_mask, blob.ptr, blob.len, com.iidOf(ID3D12RootSignature), &raw);
...
/// `D3D12_STREAM_OUTPUT_DESC`. Every field is left at its zero/null default:
/// this backend never uses stream output, and the struct is here only so
...
/// `fluxion-rhi`'s Direct3D 11 backend for where the numbers come from.
...
/// `D3D12_LOGIC_OP`. Only `clear` (zero) is ever used, and only as the
...
/// built for. Not to be confused with `d3d12_command.zig`'s
/// `PrimitiveTopology`, the finer one `IASetPrimitiveTopology` takes - a
/// pipeline built with `.triangle` accepts a list or a strip at draw time.
...
pub fn createGraphicsPipelineState(device: *ID3D12Device, desc: *const GraphicsPipelineStateDesc) Error!*ID3D12PipelineState {
    var raw: ?*anyopaque = null;
    const result = device.vtable.CreateGraphicsPipelineState(device, desc, com.iidOf(ID3D12PipelineState), &raw);
