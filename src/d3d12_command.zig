...
//! order - a renderer added later reaches for one of the `*const anyopaque`
//! ones and gives it a real signature, the same way this file did to
//! `d3d12.zig`'s stub - and only the ones this MVP actually calls are typed.
...
/// in name only - both are six `FLOAT`s in the same order - but declared fresh
/// here rather than reused, since the two libraries do not share types.
...
/// confused with `PrimitiveTopologyType` in `d3d12_pipeline.zig`, which is a
/// coarser enum a pipeline state is built with. The numbers below are
/// `D3D_PRIMITIVE_TOPOLOGY`'s, shared with Direct3D 11's `D3D11_PRIMITIVE_TOPOLOGY`
/// and with the `Topology` enum in `fluxion-rhi`'s Direct3D 11 backend.
...
/// this backend never aliases two resources over one heap.
pub const ResourceAliasingBarrier = extern struct {
    resource_before: ?*ID3D12Resource,
    resource_after: ?*ID3D12Resource,
};

/// `D3D12_RESOURCE_UAV_BARRIER`. Declared for the same reason as
/// `ResourceAliasingBarrier`; this backend has no unordered access views.
pub const ResourceUavBarrier = extern struct {
    resource: ?*ID3D12Resource,
};

/// `D3D12_RESOURCE_BARRIER`. `Type`/`Flags` are two `UINT`-sized enums ahead of
/// an anonymous union whose widest member is `Transition` (it holds a pointer
/// and rounds up to 24 bytes on its own 8-byte alignment) - so the union is
/// modelled as an `extern union` of the three real variants, which Zig lays
/// out exactly as C does, rather than as an approximation.
...
/// of `PlacedFootprint` (32 bytes, needing 8-byte alignment for its own
/// `UINT64 Offset`) and `SubresourceIndex` (a bare `UINT`) - modelled the same
/// way `ResourceBarrier`'s union is, as an `extern union` of the two real
/// shapes rather than the widest one inlined by hand.
...
pub fn createCommandAllocator(device: *ID3D12Device, kind: CommandListType) Error!*ID3D12CommandAllocator {
    ...
    const result = device.vtable.CreateCommandAllocator(device, kind, com.iidOf(ID3D12CommandAllocator), &raw);
...
pub fn createGraphicsCommandList(device: *ID3D12Device, node_mask: u32, kind: CommandListType, allocator: *ID3D12CommandAllocator) Error!*ID3D12GraphicsCommandList {
    var raw: ?*anyopaque = null;
    const result = device.vtable.CreateCommandList(device, node_mask, kind, allocator, null, com.iidOf(ID3D12GraphicsCommandList), &raw);
...
pub fn executeCommandLists(queue: *d3d12.ID3D12CommandQueue, lists: []const *ID3D12GraphicsCommandList) void {
    ...
    queue.vtable.ExecuteCommandLists(queue, @intCast(lists.len), base);
}
